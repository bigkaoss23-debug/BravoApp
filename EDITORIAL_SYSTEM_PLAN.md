...
- [x] **Catalogo Belvedere giugno COMPLETO** (2026-05-15) · 20 foto AI Higgsfield su Supabase Storage: 8 feed (1:1) + 12 stories (9:16), tutte validate al Cancello 2 · ciclo scarta→correggi prompt→rigenera testato 2 volte (feed: cerámica+taberna · stories: overhead+facciata) · **lezione strutturale appresa dal sistema**: `photo_needs.py` ora ha *vincoli per formato* — su 9:16 vietati overhead/flat-lay (composizione ruotata) e facciata generica (sembra industriale), imposta composizione verticale nativa. Il sistema impara dalla critica, non ripete l'errore
- [x] **Collegamento catalogo → Studio v2 · VALIDATO e2e** (2026-05-15) · `photo_flow.resolve_slot_photo(plan_slot_id)`: match DETERMINISTICO `photo_requests.plan_slot_id → asset photo_confirmed → client_assets.public_url`, scarica in temp · `pipeline_v2_studio.propose_post`: `photo_path` ora opzionale, se assente risolve dal catalogo via `slot.id`; se manca → errore esplicito (NON genera al volo: rimanda al flusso batch PhotoNeeds, manifesto gate umano) · test e2e: slot Belvedere 12/06 senza photo_path → Studio pesca foto dal catalogo da solo → 3 finalisti. Il flusso va da solo: piano → slot → catalogo → Studio, zero foto manuali
- [x] ~~**Bug Copy Agent ritmo_tres parsing**~~ → chiuso: `_parse` quando `json.loads` falliva metteva il JSON grezzo in `headline`. Ora 2° tentativo `json_repair` (già dip. progetto) + fallback regex per-campo (mai più JSON grezzo in headline). Testato 3/3 OK su ritmo_tres Belvedere
- [x] **Fase 1.7 · Failure Memory · VALIDATO e2e** (2026-05-15) · migration `failure_memory` (memoria dedicata) · modello: *l'umano giudica + motivo breve · l'agente struttura · il sistema previene* · **regola d'oro**: solo rifiuti espliciti con motivo (Cancello 2), MAI "non scelti" · tutto in **español** · `apply_photo_gate` scrive il rifiuto · `tools/failure_memory.py`: `distill_failures` (Sonnet vincolato: reason_raw → reason_category lista chiusa + pattern_tag + regla_es, non inventa) + `get_active_failure_rules` (blocco español per i prompt) · iniettato in **PhotoNeeds + Copy Agent** (lettura: copy_agent.run riceve `failure_rules`, pipeline_v2_studio recupera `get_active_failure_rules(client_id,"copy")`) · test e2e: rifiuto español → distill (`fuera-de-voz`) → regola pronta. Dati di test ripuliti (solo rifiuti reali di Bravo)
- [ ] **Gancio scrittura rifiuti COPY** (asimmetria nota): oggi la *lettura* failure copy è collegata al Copy Agent, ma la *scrittura* manca — lo Studio fa scegliere 1 di 3 pari grado, non c'è un "rifiuta questo copy perché X". Serve un'azione esplicita di reject copy con motivo (futuro gancio app/endpoint). NON inventato per non fare un pezzo finto
