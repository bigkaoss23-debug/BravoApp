...
- [x] **Fase 1.7 · Failure Memory · VALIDATO e2e** (2026-05-15) · migration `failure_memory` (memoria dedicata) · modello: *l'umano giudica + motivo breve · l'agente struttura · il sistema previene* · **regola d'oro**: solo rifiuti espliciti con motivo (Cancello 2), MAI "non scelti" · tutto in **español** · `apply_photo_gate` scrive il rifiuto · `tools/failure_memory.py`: `distill_failures` (Sonnet vincolato: reason_raw → reason_category lista chiusa + pattern_tag + regla_es, non inventa) + `get_active_failure_rules` (blocco español per i prompt) · iniettato in PhotoNeeds (poi Copy Agent) · test e2e: rifiuto español → distill (`fuera-de-voz`) → regola pronta. Dati di test ripuliti (solo rifiuti reali di Bravo)
