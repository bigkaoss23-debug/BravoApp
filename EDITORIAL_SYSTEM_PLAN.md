...
- [ ] **Bug tracciato · Copy Agent ritmo_tres parsing**: in propose_post il finalista `ritmo_tres` ha restituito il JSON grezzo dentro `headline` (`{ "label":"", "headline":"Terraza|sierra|taza" ...`) invece del valore parsato. Bug di robustezza `_parse` su ritmo_tres. Non lega al catalogo (quello funziona). Da fixare
